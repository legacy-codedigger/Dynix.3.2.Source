...
/* $Header: sc.h 1.2 90/11/12 $
 *
 * sc.h
 *	SSM console driver structure definitions.
 */
...
#ifdef KERNEL
#ifndef i386
extern gate_t sc_gate;
#endif i386


/* Console port control structure.  One allocated per console port. */

struct	sc_info {
	u_char	sc_minor;		/* Minor device number of this unit */
	u_short	sc_modem;		/* Modem status settings */
	u_char	sc_parsiz;		/* Parity/Character size */
	u_char	sc_iflow;		/* Input flow control method */
	u_char	sc_oflow;		/* Output flow control method */
	struct 	tty *sc_tty;		/* TTY info structure */	
	char	rx_buf[CBSIZE];		/* Received character buffer */
	sema_t	os_busy_wait; 		/* OS wait semaphore */
	u_char	os_busy_flag;		/* OS busy flag */
	struct	ssm_dev	*sc_dev;	/* Device descriptor from auto-conf */
	u_long	frame_errs;		/* Number of framing errors */
	u_long	parity_errs; 		/* Number of parity errors */
	u_long	overruns;		/* Number of overrun errors */
	u_short tx_count;		/* Number of chars to tx */
	short	devno;			/* Local/Remote port flag */
	u_char	restart_read;		/* Should new reads be started? */
};
#endif KERNEL
...
struct cons_mcb {
	...
	u_char	mcb_iflow;		/* input flow control flags */
	u_char	mcb_oflow;		/* output flow control style */
	u_char	mcb_oxoff;		/* output stop character */
	u_char	mcb_oxon;		/* output start character */
	u_char	mcb_fill[2];		/* Pad to CCB_SHSIZ bytes */
...
#define	mcb_flow	mcb_iflow	/* backwards compatible w/ old format */
...
#define	CCF_XOFF	0x01		/* XOFF/XON flow control */
#define	CCF_HWFLOW	0x02		/* hardware flow control */
...
#define	CCM_SETABLE	(CCM_BREAK | CCM_DTR | CCM_DSR | CCM_RTS)
...
/*
 * State information for /dev/console.
 * Typically a process that wishes access
 * will obtain the lock, wait on semaphore
 * until not busy, mark it as busy, release
 * the lock, use the structure (recover
 * from signals), mark it as no longer busy,
 * and awaken other waiting processes.
 */
struct sc_cons {
	int busy;			/* Is this structure in use ? */
	struct sc_info *cur;		/* Info ptr. of current console */
	struct sc_info *local;		/* Info ptr. of local cons port */
	struct sc_info *remote;		/* Info ptr. of local cons port */
	int nopen;			/* Number of outstanding opens of
					 * current console port. */
	lock_t sync;			/* For synchronizing access to
					 * this structure */
	sema_t wait;			/* For waiting to access this struct */
};

#ifdef KERNEL

/*
 * External Interfaces from ssm_cons.c.
 */
extern struct cons_cb *init_ssm_cons();
extern void init_ssm_cons_dev();
