...
#ifndef lint
static char rcsid[] = "$Header: sccs.c 2.2 91/01/03 $";
#endif
...
# ifndef PROGPATH
# define PROGPATH(name)	"name"		/* place to find binaries */
# endif PROGPATH
...
# ifndef V6
		execvp("diff", ap);
# endif
		for ( pp=execpaths; *pp; pp++ ) {
			sprintf(path, "%s%s", *pp, cmd->sccspath);
			execv(path, argv);
		}
