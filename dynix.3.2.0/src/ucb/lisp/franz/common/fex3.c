#ifndef lint
static char *rcsid = "$Header: fex3.c 1.2 1991/04/16 18:29:39 $";
#endif
...
Ndumplisp()
{
	...
	work.a_magic  = dmpmode;	/* set in data.c */
	work.a_text   = roundup((unsigned)(&etext),pagsiz);
	work.a_data   = roundup((int) sbrk(0) - ((int)curhbeg), pagsiz);
	work.a_entry  = (unsigned) gstart();
	...
	if(write(descrip,(char *)&work,sizeof(work)) != sizeof(work))
	{
		close(descrip);
		error("Dumplisp header failed",FALSE);
	}
	count = work.a_text - sizeof work - MPGSIZE;
	if( write(descrip,(char *)(sizeof work + MPGSIZE),count) != count)
	{
		close(descrip);
		error("Dumplisp text failed",FALSE);
	}
	if( write(descrip,(char *)curhbeg,(int)work.a_data) != (int)work.a_data)
	{
		close(descrip);
		error("Dumplisp data failed",FALSE);
	}
	if(des2>0  && work.a_syms) {
		if( lseek(des2,N_SYMOFF(old),0) < 0 )
		...
		for(;;) {
			...
			break;
		     ...
		     if( ax != write(descrip,tbuf,ax) )
			 error("Error in string table write ",FALSE);
		}
#endif
	}
	close(descrip);
...
{
#ifdef sequent
	return MPGSIZE;	/* 0x800 current max pagesize, for a.out */
#else
