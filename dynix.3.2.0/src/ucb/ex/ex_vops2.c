...
#ifndef lint
static char *rcsid = "$Header: ex_vops2.c 2.2 86/11/26 $";
#endif
...
vbackup:
		...
		if (trace) {
			...
			fprintf(trace, "vgetline loop: c=%c ", c);
			fprintf(trace, "gcursor=%x, ogcursor=%x, outcol=%d",
				gcursor, ogcursor, outcol);
			fprintf(trace, "inscol=%d\n", inscol);
		}
#endif
		/*
			...
			if (value(WRAPMARGIN) &&
				...
				*gcursor = 0;
				...
				if (outcol+(backsl?OCOLUMNS:0) - (gcursor-cp) >= OCOLUMNS - value(WRAPMARGIN)) {
					...
					/*
					 * If we just hit the end of vglobp,
					 * don't put '^[' into the output.
					 * -davest@sequent
					 */
					if (gcursor[-1] == ESCAPE && !lastvgk)
						*--gcursor = 0;
					/*
