...
#ifndef lint
static char *rcsid = "$Header: ex_temp.c 2.3 90/05/30 $";
#endif
		...
		if (*bp < 0) {
			...
			*bp = ((tline >> OFFBTS) & BLKMSK);
			if (*bp > NMBLKS)
				error(" Tmp file too large");
			if (bp >blocks+LBLKS)
				error("file too big, error recovery will not be possible"); 
			tline += INCRMT;
			oblock = *bp + 1;
			bp[1] = -1;
		}
	...
	int (*iofcn)();
{
	...
	if (rfile == -1) {
		CP(rfname, tfname);
		strcpy(strend(rfname) - 8, "RiXXXXXX");
		rfile = mkstemp(rfname);
		if (rfile < 0)
oops:
			filioerr(rfname);
	}
