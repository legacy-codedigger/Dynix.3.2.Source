...
#ifndef lint
static char *rcsid = "$Header: ex_set.c 2.0 86/01/28 $";
#endif
...
printone:
		...
		case OTERM:
			...
			while (!setend()) {
				...
				/* adb change:  allow whitespace in strings */
				if( (*cp = getchar()) == '\\')
					if( peekchar() != EOF)
						*cp = getchar();
				cp++;
	...
	register struct option *op;
{
	...
	case STRING:
	case OTERM:
		if ( (strlen(op->osvalue) > 16) && (how_many == 1))
			printf("%s=%.12s...", name, op->osvalue);
		else 
