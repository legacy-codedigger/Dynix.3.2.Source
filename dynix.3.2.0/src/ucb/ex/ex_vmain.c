...
#ifndef lint
static char *rcsid = "$Header: ex_vmain.c 2.4 90/05/22 $";
#endif
 ...
 * in the routine operate in ex_voper.c.
 */

#define	forbid(a)	{ if (a) goto fonfon; }

#define	ACTUAL	0		/* for two tags commands (brp) */
#define	SPECIF	1
...
reread:
		...
		/*
		**	Analyze cnt number of lines for syntactic 
		**	closure. If appropriate, close them! (brp)
		*/
		case '*':
		{
			int  aisave;
			...
			if (value(ADA) == 0) break;

			vsave();

			/* 
			** Default the cnt to 6 lines-so the n* command will almost
			** never be used. 
			*/

			if (count == 1) count = 6;

			if (count > (i = dol - dot + 1))
				count = i;
			...
			*buf = '\0';
			...
			resu = lsyn_analysis(dot, &count, buf, &indent);
			...
			if (resu > 0) {
				...
				switch (resu) {
					...
					case BODY:
							...
							if (value(AUTOINDENT) && (count > 1)) {
								...
								aisave = 1;
								pad(tmpbuf, indent);
								strcat(tmpbuf, bstring);
								pad(tmpbuf, indent);
							} else {
								strcat(tmpbuf, bstring);
							}
							strcat(tmpbuf, estring);
							if (*buf) {
								strcat(tmpbuf, " ");
								strcat(tmpbuf, buf);
							}
				...
				if(count > 1) {
					count--;
					if ((dot + count) > dol) count = dol - dot;
					getDOT();
					vmoving = 0;
					vdown(count, 0, 0);
					vnline(NOSTR);
	...
	sprintf(tmpbuf,
	"Cannot find a well-formed construct in next %d lines, try n* ?", csave);
				vsave();
				CATCH
					error(tmpbuf);
				ENDCATCH
...
pfixup:
		...
		/*
		** ^O		For Ada takes word after cursor as tag, and then does
		**   	tag command. Read ``go right to spec''. (brp)
		**		I really wanted to use ^[, but it is used as something
		**      else (^M ?).
		*/
		case CTRL(O):
			if (value(ADA) == 0) break;
...
fixup:
			...
			getDOT();
			(void)sigsetmask(oldmask); isblocked = 0;
			/*
...
short left_or_right;
{
	...
	if (*cp) {
		...
		/* read through name qualification (brp)*/
		*dp++ = 0;
	}
}
