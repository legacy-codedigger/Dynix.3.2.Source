...
#ifndef lint
static char *rcsid = "$Header: ex_subr.c 2.4 90/06/12 $";
#endif
...
/*
 * Called by system if we tried to write a file greater
 * than our current filesize limit.
 */
#ifdef SIGXFSZ
int
onxfsz()
{
	...
	error("\nFile Limit Exceeded." + inopen);
}
#endif

/*
 * Called by system if we exceeded our cpu limit.
 * Warn user and let him (try to) continue.
 */
#ifdef SIGXCPU
int
onxcpu()
 ...
 * Since we always get here via the ':' command handler and we call
 * vcontin(), no extra work is needed here to redraw the screen
 * after the SIGCONT.
 */
onsusp()
{
