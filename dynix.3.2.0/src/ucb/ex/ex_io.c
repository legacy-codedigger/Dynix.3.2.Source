...
#ifndef lint
static char *rcsid = "$Header: ex_io.c 2.7 90/05/21 $";
#endif
...
#include "ex_tty.h"
#include "ex_vis.h"
#include <a.out.h>	/* For detecting executables */
 ...
 *
 * This is a good place to do adapath stuff.  Ck all dirs on
 * the path and if the file is not found, then assume that the 
 * file is a new one.
 */
filename(comm)
	int comm;
...
/*
** Search for a file in the list of directories named in the adapath.
*/
...
/*
**	Skip leading white space and return beginning of null terminated
**	directory string.
*/
char *
get_current_dir(next_dir)
string	*next_dir;
{
	register string start = *next_dir;
	register string end;
...
/*
** Check to see if a file exists in a dir, 
** if so fix file to be a path (brp)
** and savedfile goddamnit
*/
dir_stat(dir, file)
string	dir;
string	file;
...
/*
** Look for the file in all dirs on the path (brp)
*/
ck_path(filenm)
char	*filenm;
{
	register string	current_dir;
	char	try_path[MAX_LINE_LENGTH];
	string next_dir = try_path;

	strcpy(try_path, svalue(ADAPATH));
	strcat(try_path, " ");

	/* stat the file in the current directory */
	if (dir_stat(NULL, filenm)) return;

	while((current_dir = get_current_dir(&next_dir)) != NULL) {
		if (dir_stat(current_dir, filenm)) return;
	}
}
	...
	int c;
{
	...
	/*
	 * Stat the file before we open it so that we can detect a FIFO.
	 */
	if (stat(file, &stbuf)) {
		if (errno != ENOENT)
			syserror();
...
/*
 * Given a stat buffer, return a range-checked value which would
 * be a good value for our "bsize" I/O count of bytes.
 */
long
calc_bsize(st)
	register struct stat *st;
{
	if ((st->st_blksize > VIBSIZE) || (st->st_blksize <= 0))
		return(VIBSIZE);
