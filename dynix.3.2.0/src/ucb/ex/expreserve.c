...
#ifndef lint
static char *rcsid = "$Header: expreserve.c 2.7 90/09/02 $";
#endif
	...
	int argc;
{
	...
	while ((dirent = readdir(tf)) != NULL) {
		/* Ex temporaries must begin with Vi. */
		if (dirent->d_name[0] != 'V' || dirent->d_name[1] != 'i')
			continue;
...
char	pattern[] =	usrpath(preserve/Viaa`XXXXX);
