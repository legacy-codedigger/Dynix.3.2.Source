...
#ifndef lint
static char rcsid[] = "$Header: more.c 2.13 91/04/03 $";
#endif
...
char *argv[];
{
	...
	fputs("Usage: ",stderr);
	fputs(argv[0],stderr);
	fputs(" [-cdflpsu] [-screen height] [+linenum | +/pattern] name1 name2 ...\n",stderr);
	exit(1);
    }
    else
	f = stdin;
	...
	char *fs;
{
	...
	if (fread(&ex, sizeof(ex), 1, f) == 1)
		...
		case 0411:
		case 0413:
		case 0x00ea:		/* ns32000 ?MAGIC */
		case 0x10ea:
		case 0x20ea:
		case 0x30ea:
		case 0x12eb:		/* i386 ?MAGIC */
		case 0x22eb:
		case 0x32eb:
		case 0x42eb:
		case 0177545:
...
#define ret(val) retval=val;done++;break
    ...
    if (resize) {	/* redraw screen if window changed size */
	resize = 0;
	if (!no_tty) {
		dlines = Lpp - (noscroll ? 1 : 2);
		/* clone of '\\f' code */
	...
	case 'f':
		...
		while (((c = Getc (f)) != '\n') && ++chars_read)
		    ...
		    Currline += ((int)(chars_read / Mcol) + 1);
		    nlines -= ((int)(chars_read / Mcol) + 1);
	    }
	    ret (dlines);
	case '\n':
	    if (nlines != 0)
