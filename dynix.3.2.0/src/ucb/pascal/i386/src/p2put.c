...
#if !defined(lint)
static char rcsid[] = "$Id: p2put.c,v 1.1 88/09/02 11:48:13 ksb Exp $";
#endif lint
 ...
 */
/* VARARGS */
putprintf( format , incomplete , arg1 , arg2 , arg3 , arg4 , arg5 , arg6 , arg7)
char	*format;
int	incomplete;
{
	...
	if ( !CGENNING )
		return;
	sprintf( ppbufp , format , arg1 , arg2 , arg3 , arg4 , arg5 , arg6 , arg7);
	len = strlen( ppbuffer );
	if ( len >= BUFSIZ )
		panic( "putprintf" );
	ppbufp = &( ppbuffer[len] );
    ...
    struct om	*sizesp;
{
	...
	/* ZZZZ what about floating registers */
#if defined(i386)
	maxtempreg = sizesp->curtmps.next_avail[REG_DATA];
#endif /* i386 */
	alignedframesize = roundup((int)(BITSPERBYTE * -sizesp->curtmps.om_off), (long)(BITSPERBYTE * A_STACK));
    ...
    int		type;
{
	...
	case PARAMVAR:
		...
		putleaf( PCC_NAME , (level * sizeof(savedfp_t)) + AP_OFFSET
	...
	case LOCALVAR:
		...
		putleaf( PCC_NAME , (level * sizeof(savedfp_t)) + FP_OFFSET
		    , 0 , PCCTM_PTR | PCCT_CHAR , DISPLAYNAME );
		parts[ level ] |= NONLOCALVAR;
	    }
	    putleaf( PCC_ICON , -offset , 0 , PCCT_INT , (char *) 0 );
