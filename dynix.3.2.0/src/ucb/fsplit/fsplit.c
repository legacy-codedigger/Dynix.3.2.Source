...
#ifndef lint
static char rcsid[] = "$Header: fsplit.c 1.4 87/04/16 $";
#endif
 ...
 *		after -e is optional.
 *	If -d option is specified, collect "$" compiler directives from
 *		the front of the file and include them at the front of
 *		each file created.
 *
...
/* If -d is specified, save up to 50 "$" directives from the front of the file */
#define MAX_DIRS	50
char	*directives[MAX_DIRS];
int	collecting_directives = FALSE;
int	n_directives = 0;
...
char **argv;
{
	...
	/*
	 * Write out the "$" directives, if any, saved from the
	 * beginning of the file.
	 */
	for(i=0; i < n_directives; i++)
		fprintf(ofp, "%s", directives[i]);
...
badparms()
{
	fprintf(stderr, "fsplit: usage:  fsplit [-d] [-e efile] ... [file] \n");
	exit(1);
}
...
#	define LINESIZE 80 
	...
	if(buf[0] == 'd' || buf[0] == 'D') return(0);
	ptr = buf;
	while (*ptr == ' ' || *ptr == '\t') ptr++;
	if(*ptr == '\n') return(0);

	/*
	 * Check for a Sequent-style "$" compiler directive.
	 * If we're still at the front of the file, save it.
	 * Otherwise treat it as a comment.
	 */
	if(buf[0] == '$') {
		if (collecting_directives) {
			if (n_directives >= MAX_DIRS) {
				fprintf(stderr,
"fsplit: limit of %d global \"$\" compiler directives exceeded.\n", MAX_DIRS);
				exit(1);
			}
			directives[n_directives] = malloc((unsigned)strlen(buf)+1);
			(void) strcpy(directives[n_directives],buf);
			n_directives++;
		}
		return(0);
	}

	/* Not a comment or compiler directive. */
	collecting_directives = FALSE;
