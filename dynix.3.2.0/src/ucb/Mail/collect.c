...
#ifndef	lint
static char *rcsid = "$Header: collect.c 2.2 87/04/02 $";
#endif
...
{
	FILE *ibuf, *fbuf, *obuf;
	int lc, cc, escape, collrub(), intack(), collhup, colltstp(), eof;
	register int c, t;
...
# ifdef VMUNIX
	...
	savetstp = sigset(SIGTSTP, colltstp);
# else VMUNIX
...
# ifdef VMUNIX
	savestop = sigset(SIGTSTP, SIG_DFL);
# endif VMUNIX
	fflush(obuf);
	rewind(ibuf);
