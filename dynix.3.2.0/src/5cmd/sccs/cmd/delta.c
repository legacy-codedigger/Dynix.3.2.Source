...
char *file;
{
	...
	Did_id = 0;
	while (afgets(line,sizeof(line),Gin) != NULL &&
...
struct sid *sp;
{
	...
	while (afgets(line,sizeof(line),in) != NULL) {
		pf_ab(line,&pf,1);
...
register int n;
{
	...
	if ((r = afgets(s,n,Diffin)) != NULL && HADP)
...
register struct packet *pkt;
{
	...
	iptr = xfopen(file,0);
	for (k = 1; afgets(line,sizeof(line),iptr); k++)
		if (*line == CTLCHAR) {
			fclose(iptr);
