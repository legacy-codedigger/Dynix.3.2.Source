...
#ifndef	NROFF
/*
 * Sequent didn't supply the "real" default, aps.  For customers who got
 * the font set themselves, we leave the default alone.  For others, we
 * switch the default to the one Sequent actually provided, "i10".
 */
set_default()
{
	char buf[40];
	...
	sprintf(buf, "%s/%s", fontfile, devname);
	if (stat(buf, &sb) < 0)
		strcpy(devname, "i10");
}
#endif
