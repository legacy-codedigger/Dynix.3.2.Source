...
#ifdef RCS
static char rcsid[]= "@(#)$Header: zdverify.c 1.9 91/03/26 $";
#endif
...
verify()
{
	...
	npat = 0;
	if (doverify) printf("Starting verify passes.\n");
	/*
	 * Set for severe burn-in.
	 */
	ioctl(fd, SAIOSEVRE, (char *)NULL);
	...
	tracksize = chancfg->zdd_sectors << DEV_BSHIFT;
        ...
        for (hdrpassnum = 1; hdrpassnum <= hdrpasses; hdrpassnum++) {
                ...
                if (dohdrpass() == FAIL) {
                        printf("Errors found - adding an additional header verify pass.\n");
                        hdrpasses++;
                }
        }
        if (!doverify) goto verify_done;
	...
	for (pass = 0; pass < fullpasses; pass++) {
		...
		if (dofullpass(tracksize, npat) == FAIL)
			printf("Warning: Defect(s) found during full pass %d\n",
				pass + 1);
		if (++npat > pps)
			npat = 0;
	}
	...
	for (pass = 0; pass < defectpasses; pass++) {
		...
		if (dodefectpass(tracksize, npat) == FAIL) {
			defectpasses++;
			printf("Error found - Adding an additional Defect verify pass.\n");
		}
		if (++npat > pps)
			npat = 0;
	}
	...
	struct	cyl_hdr *chp;
{
	...
	if (verbose) {
		printf("Pounding track at (%d,%d). This takes about %d minutes\n", 
			cyl, track, chancfg->zdd_sectors);
	}
	...
	int length;
{
	...
	lseek(fd, sector << DEV_BSHIFT, L_SET);
	if (write(fd, cbp, length) == length)
		return;

	for (sn = sector; sn < sector + (length >> DEV_BSHIFT); 
	     sn++, cbp+=DEV_BSIZE) {
		lseek(fd, sn << DEV_BSHIFT, L_SET);
		(void) write(fd, cbp, DEV_BSIZE);
	}
...
again:
	...
	if (!checkdata) {
		...
		bzero((caddr_t)cbp, (u_int)length);
		errno = 0;
		if ((ln = read(fd, cbp, length)) == length) {
			/*
	...
	for (sn = sector; sn < sector + (length >> DEV_BSHIFT); sn++) {
		for (i = ZDNBEATS; i > 0; i--) {
			...
			if (cc == DEV_BSIZE) {
				...
				printf("read error at (%d,%d,%d)\n",
					cyl, trk, sect+sn-sector);
	...
	caddr_t	rcbp;			/* buffer with pattern to read  */
{
	...
	lcb.cb_addr = (u_long)wcbp;
	lcb.cb_count = DEV_BSIZE;
	lcb.cb_mod = 0;
	lcb.cb_iovec = 0;
	/* ignore write errors */
	(void)ioctl(fd, SAIOZDCCMD, (char *)&lcb);
	...
	struct	hdr	*hp;
{
	...
	(void)ioctl(fd, SAIONSEVRE, (char *)NULL);

	printf("Zdc error %d on read.\n", errcode);
	cnterr.all++;
	 ...
	 */
	if (errcode == EHDRECC || errcode == EBSE || errcode == ESO) 
	 ...
	 */
	bzp = find_bbl_entry(bbl, cyl, (u_char)trk, (u_char)sn);
	/*
	...
	||  (bzp->bz_rtype == BZ_AUTOREVECT && errcode == EHDRECC)) {
...
fail_drive(cyl, head, sect)
	...
	int sect;
{
	printf("WARNING: Uncorrectable hdr-ecc ");
	if (sect < 0) 
		printf("problems on track (%d, %d).\n", cyl, head);
	else
		printf("problems with sector (%d, %d, %d).\n", 
			cyl, head, sect);

	if (sect == n_hdrs-1 && n_hdrs != totspt)
		printf("WARNING: Defective runt sector.\n");

	printf("WARNING: *** Defective drive or drive channel ***.\n");
	...
	int sect;
{
	...
	if (suspect_id < 0 || suspect_id >= num_suspects) {
                printf("adjust_header: internal error...exiting.\n");
		exit(1);
	}
	...
	pattern = (sp->adjustment[sect] & 1) ? ZDALTPAT : ZDSYNCPAT;
	if (verbose) {
	 ...
	 */
        if (!buf) {
		callocrnd(DEV_BSIZE);
                buf = (u_char *) calloc(CNTMULT+DEV_BSIZE);
	}
	...
	cb.cb_cmd = ZDC_SET_CHANCFG;
        cb.cb_addr = (u_long)chancfg;
        cb.cb_count = sizeof(struct zdcdd);
        cb.cb_iovec = 0;
        if (ioctl(fd, SAIOZDCCMD, (char *)&cb) < 0) {
        	printf("ZDC_SET_CHANCFG ioctl failed while fixing header");
		exit(1);
	}
	...
	bzero((caddr_t)&cb, sizeof(struct cb)); 
  	...
  	cb.cb_head = head;
  	cb.cb_sect = sect;
  	cb.cb_addr = (u_long)buf;
  	cb.cb_count = CNTMULT + DEV_BSIZE;
  	cb.cb_iovec = 0;
        if (ioctl(fd, SAIOZDCCMD, (char *)&cb) < 0) {
        	printf("ZDC_WHDR_WDATA ioctl failed while fixing header");
		exit(1);
	}
	sp->adjustment[sect]++;
	...
	*chancfg = save_cfg;
	...
	cb.cb_cmd = ZDC_SET_CHANCFG;
        ...
        cb.cb_iovec = 0;
        if (ioctl(fd, SAIOZDCCMD, (char *)&cb) < 0) {
        	printf("ZDC_SET_CHANCFG ioctl failed during restore");
		exit(1);
	}
}
	...
	register u_char head;
{
	...
	if (num_suspects == max_suspects) {
		printf("WARNING: Cannot test sector headers on ");
		printf("cyl %d, head %d - list full.\n", cyl, head);
		return(-1);
	}

	if (verbose && task != ADDBAD) 
	    printf("Adding track (%d, %d) to header verify list (entry %d).\n",
			cyl, head, num_suspects);

	sp->addr.da_cyl = cyl;
	sp->addr.da_head = head;
	sp->adjustment = (short *) calloc(n_hdrs * sizeof(short));
	if (!sp->adjustment) {
		printf("add_suspect: memory allocation failure - exiting.\n");
		exit(1);
	}
	return(num_suspects++);
}
	...
	u_char	head;
{
	...
	struct cb lcb;
		...
		lcb.cb_cyl = cyl;
		lcb.cb_addr = (u_long)headers;
		lcb.cb_head = head;
		lcb.cb_count = size;
		lcb.cb_iovec = 0;
		errno = 0;
		status = ioctl(fd, SAIOZDCCMD, (char *)&lcb);
		errcode = errno;
                if (verbose && status < 0 && retry) {
                        printf("ZDC_READ_HDRS ioctl error -");
                        printf("Attempting recovery by retrying ioctl.\n");
                }
        }

	if (status == -1) {
		if (verbose && errcode != EHER) {
			printf("(%d,%d) ", cyl, head);
			printf("ZDC_READ_HDRS ioctl error\n");
			printf("errcode 0x%x\n", errcode);
		}
...
validate_hdrs(suspect_id)
	int suspect_id;
{
	u_short cyl = suspect[suspect_id].addr.da_cyl;
	...
	for (pass = 0, i = ZDNBEATS; i > 0; i--, pass++, changes_made = 0) {
		...
		if (read_hdrs(cyl, head)) {
			...
			if (errcode == EHER) {
				if (verbose) {
					printf("Possible problem at (%d,%d) - trying data sync fix\n", 
						cyl, head);
				}
...
ok:
			...
			addbad(ndefects, VERIFY);
                	(void)ioctl(fd, SAIOSEVRE, (char *)NULL);
...
dohdrpass()
{
	...
	for (; s < num_suspects; s++)
		...
		if (verbose) {
		       printf(
		       "header pass %d: corrections made to cyl %d, head %d.\n",
...
dump_hdrs()
{
	...
	for (j = 0; j < n_hdrs; j++) {
		...
		if ((j % 4) == 0) 
			printf("%x:", j);
		printf("  %x %x %x %x %x", rhp->h_type, rhp->h_flag, 
...
dump_hdr(hp, rhp, sect)
	...
	int	sect;
{
	printf("  Expect %x %x %x %x %x, ",
		hp->h_type, hp->h_flag, hp->h_cyl, hp->h_head, hp->h_sect);
	printf("read %x %x %x %x %x @ physical 0x%x\n",
