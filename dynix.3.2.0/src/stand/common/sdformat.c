...
/* $Header: /usr/src/dynix/src/stand/common/RCS/sdformat.c,v 1.1 1993/12/06 18:34:45 bruce Exp $ */

/* $Log: sdformat.c,v $
 * Revision 1.1  1993/12/06  18:34:45  bruce
 * Initial revision
 *
 *
 */
...
/*
 * sdformat - standalone formatter for Common Command Set (CCS) SCSI disks
 *
 */
	...
	*pp-- = '\0';
	...
	while (fd < 0) {
		...
		if ((cp[0] == 's' && cp[1] == 'd')
			...
			printf("Example for target adaptor 5: sd(40,0)\n");
			continue;
		}
	}
}
...
is_CCS_disk()
{
	...
	if (ioctl(fd, SAIOSCSICMD, (char *)&inqarg) < 0) {
		printf("sdformat: problems doing INQUIRY\n");
		exit(1);
	}

	if (inqarg.inq.sdq_format == 0) {
		printf("sdformat: This is not a supported CCS disk.  ");
 ...
 * product IDs in the drive_table to determine the geometry.  Also,
...
get_disk_type()
{
	...
	if (ioctl(fd, SAIOSCSICMD, (char *)&inqarg) < 0) {
		printf("sdformat: failed on INQUIRY command\n");
		exit(1);
	}
	...
	if (!dp) {
		printf("sdformat: could not find ID string in drive table\n");
		exit(1);
	}
	...
	if (ioctl(fd, SAIOSCSICMD, (char *)&readcarg) < 0) {
		printf("sdformat: failed on READ CAPACITY command\n");
		exit(1);
	}
...
int block;
{
	...
	if (ioctl(fd, SAIOSCSICMD, (char *)&reassarg) < 0) {
		printf("sdformat: problem with REASSIGN BLOCKS\n");
		return(-1);
	}
	return(0);
}
...
write_diag()
{
	...
	if (ioctl(fd, SAIODEVDATA, (char *)&st) < 0) {
		printf("sdformat: can't get device data\n");
		exit(1);
	}
	...
	if (!db_buf) {
		printf("sdformat: calloc failed.  No buffer for patterns.\n");
		exit(1);
	}
	...
	if (lseek(fd, diag_start * 512, 0)) {
		printf("sdformat: Can't seek to diagnostic block %d\n",
			diag_start);
		exit(1);
	}
	...
	for (lba = diag_start; lba < diag_end; lba += n) {
		...
		if ((count = write(fd, (char *)db_buf, i)) != i) {
			if (count > 0)
				lba += count / sizeof(*db);
			printf("sdformat: Error writing diagnostic block %d\n",
				lba);
			break;
		}
	}
