...
%{
static char rcsid[] = "$Header: egrep.y 2.2 91/03/28 $";
#include <stdio.h>
...
char *file;
{
	...
	p = buf;
	nlp = p;
	if ((ccount = read(f,p,MAXBUFSIZE/2))<=0) goto done;
	...
	for (;;) {
		...
		found:	for(;;) {
				...
				succeed:	nsucc = 1;
						if (cflag) tln++;
							...
							if (p <= nlp) {
								while (nlp < &buf[MAXBUFSIZE]) putchar(*nlp++);
								nlp = buf;
							}
				...
				cfound:
				if (--ccount <= 0) {
					if (p <= &buf[MAXBUFSIZE/2]) {
						if ((ccount = read(f, p, MAXBUFSIZE/2)) <= 0) goto done;
					}
					else if (p == &buf[MAXBUFSIZE]) {
						p = buf;
						if ((ccount = read(f, p, MAXBUFSIZE/2)) <= 0) goto done;
					}
					else {
						if ((ccount = read(f, p, &buf[MAXBUFSIZE]-p)) <= 0) goto done;
					}
					blkno++;
				}
			}
		...
		if (--ccount <= 0) {
			if (p <= &buf[MAXBUFSIZE/2]) {
				if ((ccount = read(f, p, MAXBUFSIZE/2)) <= 0) break;
			}
			else if (p == &buf[MAXBUFSIZE]) {
				p = buf;
				if ((ccount = read(f, p, MAXBUFSIZE/2)) <= 0) break;
				...
				if ((ccount = read(f, p, &buf[MAXBUFSIZE] - p)) <= 0) break;
			}
			blkno++;
		}
	}
