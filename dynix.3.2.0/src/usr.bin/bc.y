...
%{
static char rcsid[] = "$Header: bc.y 2.3 90/04/03 $";
	int *getout();
%}
%right '='
%left '+' '-'
...
char **argv;
{
	...
	/*
	 * Fake in library argument for 'l' flag
	 */
	if (lflag) {
		x -= 1;
		argv[x] = "/usr/lib/lib.b";
	}

	/*
	 * If compile-only, just run the parser/translator
	 */
	if (cflag) {
		x -= 1;
		yyinit(argc-x, argv+x);
		yyparse();
		exit(0);
	}

	/*
	 * Otherwise start up dc, and arrange to drive it
	 * from our compiler
	 */
	pipe(p);
	if (fork()==0) {
