...
#ifndef lint
static char rcsid[] = "$Header: tip.c 2.3 1991/05/16 21:29:43 $";
#endif
	...
	char *argv[];
{
	...
	for (; argc > 1; argv++, argc--) {
		...
		case 'p':
			...
			if (equal(argv[1],"none")) {
				strcpy(parity,"none");
				break;
			}
			if (equal(argv[1],"zero")) {
				strcpy(parity,"zero");
				break;
			}
			if (equal(argv[1],"one")) {
			...
			if (equal(argv[1],"ignore")) {
				strcpy(parity,"ignore");
				break;
			}
			usage();
		case 'v':
...
#ifdef	DEBUG
	printf("parity is %s vflag is %d\n",parity,vflag);
	printf("Speed is %d system is %s\n",BR,system);
#endif
...
notnumber:
	 ...
	 *  return to the real uid.  These things will
	 *  be closed on exit.  Note that we can't run as root,
	 *  because locking mechanism on the tty and the accounting
	 *  will be bypassed.
	 *
	 *  Leaving the set-gid bit still effective is needed to
	 *  remove our lock file.  Be sure /usr/spool/uucp is mode 775,
	 *  owner uucp, group daemon and tip is 6775, owner uucp, group daemon.
	 */
	setuid(getuid());	/* from "uucp" to real user id */
	...
	setparity(parity);			/* set the parity table */
	...
	register int n;
{
	...
	bp = buf;
	/* We only mess with the chars if parity matters, otherwise pass
	 * them on unmodified because all 8bits are meaningful
	 */
	if (!equal(parity,"ignore")) {
		for (i = 0; i < n; i++) {
			*bp = partab[(*bp) & 0177];
			bp++;
		}
	}
	if ((i = write(fd, buf, n)) != n) {
		if (i < 0 && errno == EIO)
			abort("Lost carrier.");
		printf("\r\nwrite error\r\n");
		finish();
	}
	if (kill(pid, 0))	/* child died */
		finish();
}
	...
	char *defparity;
{
	...
	if (equal(parity, "ignore")) {		/* do not modify char */
		return;				/* all 8 bits are meaningful*/
	}
...
usage()
{
	fprintf(stderr, "usage: tip [-v] [-speed] [-p parity] [system-name]\n");
	exit(1);
}
