...
#ifndef lint
static char rcsid[] = "$Header: sa.c 2.3 1991/09/24 16:58:53 $";
#endif
	...
	double n, a, b, c, d, e;
{
	...
	col(n, a, treal, "re");
	if (oflg)
		col(n, 60*HZ*(b/(bc)), tcpu+tsys, "u/s");
	else if(lflg) {
		col(n, b, tcpu, "u");
		col(n, c, tsys, "s");
	} else
	...
	char *cp;
{
	...
	if(jflg)
		printf("%11.2f%s", a/(n*(double)HZ), cp); else
		printf("%11.2f%s", a/(60.*(double)HZ), cp);
	if(cflg) {
		if(a == m)
			printf("%9s", ""); else
			printf("%8.2f%%", 100.*a/m);
	...
	while (fread((char *)&fbuf, sizeof(fbuf), 1, ff) == 1) {
		...
		if (uflg) {
			printf("%3d %6.2f cpu %8luk mem %6ld io %.*s\n",
			    fbuf.ac_uid, x/(double)HZ, y, z, NC, fbuf.ac_comm);
			continue;
		}
	...
	cell *p1, *p2;
{
	...
	f1 = sum(p1) / (p1->p.count?p1->p.count:1.0);
	f2 = sum(p2) / (p2->p.count?p2->p.count:1.0);
	if(f1 < f2) {
		if(rflg)
			return(-1);
		return(1);
