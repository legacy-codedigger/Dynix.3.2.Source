...
#ifndef	lint
static	char	rcsid[] = "$Header: tcp_output.c 2.9 1991/09/11 01:05:24 $";
#endif

/*
 * tcp_output.c
 * 	TCP output figure out what should be sent and send it.
 */
...
send:
	...
	if (m == NULL) {
		/*
		 * we had to send but couldn't due to buffer space.  Arrange
		 * for a retransmission to take care of this send and mask
		 * the error
		 */
...
#define	MAXLINKHDR	16		/* belongs elsewhere */
