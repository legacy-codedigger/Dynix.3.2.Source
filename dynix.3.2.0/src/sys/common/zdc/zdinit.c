...
#ifndef	lint
static	char	rcsid[] = "$Header: zdinit.c 1.29 91/01/14 $";
#endif
...
extern	u_int	zdccmdtime;		/* polled command timeout value */
extern	u_int	zdcinitime;		/* init ctrlr timeout value */
...
#ifdef MIRRORTEST
extern  lock_t	zdfaillock;		  /* mirror failure lock */
...
lock_t	zdcprlock;			/* lock for error message sync */
...
conf_zdc()
{
	register struct ctlr_toc *toc = &CD_LOC->c_toc[SLB_ZDCBOARD];
	register struct	ctlr_desc *zdcfg = &CD_LOC->c_ctlrs[toc->ct_start];
...
#ifdef MIRRORTEST
	init_lock( &zdfaillock, mirrorgate );
#endif /* MIRRORTEST */
	CPRINTF("%d ZDC%s; slic", numzdc, PLURAL(numzdc));
	...
	for (ctlrp = zdctrlr; ctlrp < &zdctrlr[numzdc]; ctlrp++, zdcfg++) {
		...
		ctlrp->zdc_diagflag = zdcfg->cd_diag_flag;
		if (ctlrp->zdc_diagflag & (CFG_FAIL|CFG_DECONF)) {
			ctlrp->zdc_state = ZDC_DEAD;
			if (ctlrp->zdc_diagflag & CFG_FAIL) {
				ERRLIGHTON;
			}
		} else {
	...
	if (alivezdc < numzdc) {
		printf("Not using ZDC: slic");
                /*
                 *+ The DCCs listed did not pass power-up
                 *+ diagnostics or have been deconfigured.
...
probe_zdc_devices()
{
	...
	struct	cb lcb;
	int	numzdc = CD_LOC->c_toc[SLB_ZDCBOARD].ct_count;
	struct v_open *vo;
		...
		for (;;) {
			...
			printf("zdc%d at slic %d Deconfigured.\n",
				ctlrp - zdctrlr, ctlrp->zdc_slicaddr);
                        /*
                         *+ The specified DCC controller could not be
                         *+ initialized.  Corrective action:  call service.
                         */
			ctlrp->zdc_state = ZDC_DEAD;
			ERRLIGHTON;
			break;
	 ...
	 * set for max_RAW_IO.
	 */
	if (zdc_iovpercb <= 0 || ((zdc_iovpercb * CLBYTES) > max_RAW_IO))
		zdc_iovpercb = (max_RAW_IO + CLOFSET) / CLBYTES;
	...
	for (ctlrp = zdctrlr; ctlrp < &zdctrlr[numzdc]; ctlrp++) {
		...
		if (pollzdcmd(ctlrp, cbp, CHAN_A) < 0) {
			...
			printf("zdc%d at slic %d Deconfigured.\n",
					ctlrp - zdctrlr, ctlrp->zdc_slicaddr);
                        /*
                         *+ The specified DCC controller did not respond
                         *+ to the probe command.  Corrective action:
                         *+ call service.
                         */
			ctlrp->zdc_state = ZDC_DEAD;
			ERRLIGHTON;
		...
		for (ctrlr = 0; ctrlr < numzdc; ctrlr++) {
			...
			if (ctlrp->zdc_state != ZDC_ALIVE) {
				printf("zd%d on dead controller zdc%d.\n",
							up - zdunit, ctrlr);
				/*
				 *+ The specified drive is on a controller 
				 *+ that has either failed diagnostics or has
				 *+ been declared dead during initialization.
				 *+ Corrective action:
				 *+ call service.
                         	 */
				up->zu_state = ZU_BAD;
			...
			for (drive = 0; drive < ZDC_MAXDRIVES; drive++) {
				 ...
				 */
				found[ctrlr] |= 1 << drive;
				CPRINTF("zd%d bound to zdc%d drive %d.\n",
				...
				/* Allocate space for VTOC info */
				vo = (struct v_open *)
					calloc(sizeof(struct v_open));
				up->zu_part = &vo->v_v;
				up->zu_opens = &vo->v_vo.v_opens[0];
				up->zu_modes = &vo->v_vo.v_modes[0];
				...
				if (up->zu_cfg == ZD_NOTFOUND ||
					...
					printf("zd%d: drive not present.\n",
						up - zdunit);
					/*
					 *+ The specified zd drive is not 
					 *+ responding.  It might not be online
					 *+ or be powered up. Corrective action:
					 *+ make sure the drive is powered
					 *+ up and online.  If the drive is 
					 *+ online, powered up, and still does
					 *+ not respond, call service.
					 */
	...
	int	channel;
{
	...
	if (pollzdcmd(ctlrp, cbp, channel) < 0) {
		...
		printf("zdc%d at slic %d Deconfigured.\n",
				ctlrp - zdctrlr, ctlrp->zdc_slicaddr);
                /*
                 *+ The specified DCC controller is not responding
                 *+ to commands.  Corrective action:  call service.
                 */
	...
	register int ctlr;
{
	...
	printf("zdc%d at slic %d Deconfigured.\n", ctlr,
        ...
        /*
         *+ The specified DCC controller has failed configuration or
         *+ initialization.  Corrective action:  call service.
         */
	any = 0;
	...
	int	numzdc;
{
	...
	for (; ctlrp < &zdctrlr[numzdc]; ctlrp++, errvec++, cbvec += NCBPERZDC){
		 ...
		 */
		if (CD_LOC->c_sys_mode.sm_bus_mode == CD_BUS_EXTENDED_NARR ||
		    CD_LOC->c_sys_mode.sm_bus_mode == CD_BUS_EXTENDED_WIDE)
		...
		if (zdctrl & ZDC_DUMPONPANIC) {
			 ...
			 */
			callocrnd(CLBYTES);	/* align at 1K boundary */
		...
		if (pollzdcmd(ctlrp, (struct cb *)icbp, CHAN_A) < 0) {
			printf("zdc%d: Cannot init zdc.\n", ctlrp - zdctrlr);
                        /*
                         *+ The specified DCC controller did not respond to
                         *+ an initialization command.  Corrective action:
                         *+ call service.
                         */
			ctlrp->zdc_state = ZDC_DEAD;
			ERRLIGHTON;
...
retry:
	...
	while (cbp->cb_compcode == ZDC_BUSY) {
		if (--i == 0) {
			...
			printf("zdc%d: Cmd %x timeout.\n", ctlrp - zdctrlr,
								cbp->cb_cmd);
                        /*
                         *+ The specified DCC controller timed out waiting
                         *+ for a command to complete.  Corrective action:
                         *+ call service.
                         */
			val = rdslave(ctlrp->zdc_slicaddr, SL_Z_STATUS);
			if ((val & SLB_ZPARERR) ||
				...
				printf("zdc%d: Ctrlr error status 0x%x.\n",
					ctlrp - zdctrlr, val);
                                /*
                                 *+ The specified DCC controller gave an
                                 *+ unexpected error condition while
                                 *+ processing a command.  Corrective
                                 *+ action:  call service.
                                 */
				/*
				 * controller bad!
	...
	case ZDC_CH_RESET:
		...
		printf("zd%d: Drive failed during cmd 0x%x.\n",
                ...
                /*
                 *+ The specified drive failed to complete a command.
                 *+ Corrective action:  call service.
                 */
		goto hard;
	...
	case ZDC_ACCERR:
		...
		printf("zd%d: Access error 0x%x on transfer at 0x%x.\n",
                ...
                /*
                 *+ The specified zd drive had an access error.  This is
                 *+ probably a controller problem.  Corrective action:
                 *+ call service.
                 */
	...
	default:
		...
		printf("zdc%d: Bad compcode 0x%x.\n", ctlrp - zdctrlr,
			cbp->cb_compcode);
                /*
                 *+ The specified DCC controller returned a completion code
                 *+ that is unknown to the driver.  Corrective action: call
                 *+ service.
                 */
	...
	if (acbp->cb_errcnt++ < zdcretry) {
		...
		if (pollzdcmd(ctlrp, &lcb, drive) < 0) {
			printf("zd%d: RESET failed.\n", cbp->cb_unit);
                        /*
                         *+ The specified zd drive failed to complete the
                         *+ RESET command, which was issued after another error
                         *+ occurred.  Corrective action:  call service.
                         */
			goto hard;
		}
		goto retry;
...
hard:
	...
	printf("zd%d: Hard error (%s); cmd = 0x%x at (%d, %d, %d).\n",
		...
		cbp->cb_cyl, cbp->cb_head, cbp->cb_sect);
        /*
         *+ The specified drive had a hard error on the specified command
         *+ at (cylinder, head, sector).  Corrective action:  if the location
         *+ where the hard error occurs is constant, use the format
         *+ command to add the sector to the bad block list.  If multiple
         *+ locations are getting hard errors, call service.
         */
