...
#ifndef	lint
static char rcsid[] = "$Header: portmap.c 1.6 91/02/28 $";
#endif
...
main()
{
	...
	if (ndescriptors < GOODSIZE) {
		ndescriptors = setdtablesize(GOODSIZE);
		if (ndescriptors == -1) {
			perror("portmap: setdtablesize");
			exit(1);
		}
	}
	pid = fork();
	...
	SVCXPRT *xprt;
{
	...
	static char buf[10*1024]; /* big enough for largest set of args */
