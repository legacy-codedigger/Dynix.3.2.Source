...
%{#include "defs"
static char rcsid[] = "$Header: gram.y 2.1 89/12/18 $";
%}
	...
	if (fin == NULL) {
		...
		for (p = text = yytext ; p < yytextl ; *p++ = kc) {
			...
			case '\n':
				...
				/*
				 * If this isn't a line ending with '\',
				 * then pass off the whole line
				 */
				if ((p==yytext) || (p[-1] != '\\')) {
					*p = '\0';
					goto endloop;
				...
				/*
				 * Otherwise turn the '\' into a blank and
				 * arrange to accumulate the next line into
				 * the buffer, too.
				 */
				p[-1] = ' ';
				for (;;) {
				...
				/*
				 * Blank line followed; end of line
				 * after all
				 */
				if (kc == '\n') {
					*p = '\0';
				...
				if(kc != EOF)
					break;
				/* ---VVV Fall into EOF case below VVV--- */
			case EOF:
				*p = '\0';
				return(0);
			}
