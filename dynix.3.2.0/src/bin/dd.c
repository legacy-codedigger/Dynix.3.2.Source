...
#ifndef lint
static char rcsid[] = "$Header: dd.c 2.2 1991/07/12 00:44:38 $";
#endif
...
char	**argv;
{
	...
	ibuf = sbrk(ibs + (pagesize-1));
	if (fflag)
		obuf = ibuf;
	else
		obuf = sbrk(obs + (pagesize-1));
	...
	/*
	 * align to pagesize boundary.
	 */
	ibuf = (char *)(((int)ibuf + (pagesize-1)) & ~(pagesize-1));
	obuf = (char *)(((int)obuf + (pagesize-1)) & ~(pagesize-1));
