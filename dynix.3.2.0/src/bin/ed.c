...
#ifndef lint
static char rcsid[] = "$Header: ed.c 2.2 90/03/30 $";
#endif
...
#include <setjmp.h>
#define	NULL	0
#define	FNSIZE	1024	/* really should be MAXPATHLEN */
#define	LBSIZE	512
#define	ESIZE	128
#define	GBSIZE	256
#define	NBRA	5
...
# ifdef	CRYPT
			xflag = 1;
# else
#define	ERR_MSG 	"ed: -x flag not supported by this version.\n"
...
# ifdef	CRYPT
		...
		kflag = crinit(key, perm);
# else
		puts("Encryption mode not available in this version.");
# endif	CRYPT
		continue;
...
getkey()
{
	...
	for(p = getpass("Key:"), k = &key[0]; *p && (k < &key[KSIZE]);)
		*k++ = *p++;
	*k = 0;
	return(key[0] != 0);
}
