...
{
	if (iop->_file != __fr_adj_val_for_NFILE) {
...
register FILE	*iop;
{
	...
	/* dummy file descriptors (iop->_file = __fr_adj_val_for_NFILE), */
	...
	bufptr = iop->_ptr;
	bufferend = (fno == __fr_adj_val_for_NFILE) ? 
	...
	for ( ; ; ) {
		...
		if (fcode == '\0') {  /* end of format; return */
			...
			if (bufptr + iop->_cnt > bufferend && /* in case of */
					iop->_file != __fr_adj_val_for_NFILE) /* interrupt */
